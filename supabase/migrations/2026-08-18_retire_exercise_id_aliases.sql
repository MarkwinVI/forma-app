-- Retire the app-side exercise-id alias table.
--
-- Skill-tree steps have been renamed over time, and until now the app read
-- old ids as new ones on the way in while writing everything under the new
-- id. That left one step stored as two rows and the app reconciling them —
-- a moving part with nothing to recommend it. From here on an id is
-- immutable, a removed step is a data migration written when it is removed,
-- and the app reads and writes one id with nothing to translate.
--
-- This converts every row still stored under an old id. Where more than one
-- row ends up naming the same step for a user — the new id already had one,
-- or two old ids became the same new id — the most recently updated is kept
-- and the rest dropped: there are no active users at the time of writing, so
-- nothing here carries state worth merging.

create temporary table renamed_steps (old_id text, new_id text) on commit drop;
insert into renamed_steps values
...
-- Progress: one row per (user, step) — however many ids named that step.
-- Several old ids can point at one new id, and the new id may have a row of
-- its own, so every row is first labelled with the id it is going to have;
-- then, per user and per step, only the most recently updated row survives.
with labelled as (
  select p.id,
         p.user_id,
         coalesce(r.new_id, p.exercise_id) as target_id,
         p.updated_at
  from public.user_exercise_progress p
  left join renamed_steps r on r.old_id = p.exercise_id
),
ranked as (
  select id,
         row_number() over (
...
delete from public.user_exercise_progress p
using ranked
where ranked.id = p.id
  and ranked.rank > 1;

update public.user_exercise_progress p
set exercise_id = r.new_id
from renamed_steps r
where p.exercise_id = r.old_id;

-- The ledger and the workout log carry no uniqueness on the id, so they
-- simply rename.
update public.progression_events e
set exercise_id = r.new_id
from renamed_steps r
where e.exercise_id = r.old_id;

update public.progression_events e
set related_exercise_id = r.new_id
from renamed_steps r
where e.related_exercise_id = r.old_id;

update public.workout_exercise_logs x
set exercise_id = r.new_id
from renamed_steps r
where x.exercise_id = r.old_id;

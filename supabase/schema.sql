...
-- ── Feedback ──────────────────────────────────────────────────────────────
-- One row per thing a user told us: the session rating that closes every
-- workout (a thumb, reason tags, flagged exercises, an optional note), and
-- later the app rating and support messages. Each row carries the build it
-- was sent from. The note lives only here — analytics gets the tags.
...
create table public.feedback (
  ...
  flagged_exercise_ids  text[] default '{}' not null,
  note                  text,
  app_version           text,
  platform              text,
  os_version            text,
